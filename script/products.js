...
let padreCards = document.querySelector(".galeria1");
const insertarProductos = (contenedor, listaProductos) => {
  contenedor.innerHTML = "";
  listaProductos.forEach((producto) => {
    contenedor.innerHTML += `
    <div class="fila1" data-tipo="${producto.tipoAccesorio}" name=${producto.id}>
...
const recorrerCard = () => {
  const cards = document.querySelectorAll(".fila1");
  cards.forEach((elemento) => {
    elemento.addEventListener("click", () => {
      const idCar = elemento.getAttribute("name");
